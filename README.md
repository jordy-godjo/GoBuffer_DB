...
## 7 — Remise des éléments au jury (format recommandé)

Préparez un archive ZIP (ou tar.gz) contenant :

- `minisgbd` (ou `minisgbd.exe` pour Windows) ou instructions pour `go build`
- `config.txt`
- `commands.txt`
- `Fruit_extra.csv`
- `expected.txt`
...
Ajoutez une note courte : "Exécutez `./minisgbd -config config.txt < commands.txt > output.txt` puis comparez `output.txt` avec `expected.txt`. Pour toute anomalie, supprimez `data/` et relancez (voir Guide)".

## 8 — Options que je peux ajouter (si vous voulez)

- `-fresh` ou commande `CLEARPERSIST` pour réinitialiser l'état sans supprimer manuellement `data/`.
- Parser CSV robuste basé sur `encoding/csv` (recommandé si vos CSV sont compliqués).
- Script d'automatisation `tools/run_readme_scenario.sh` (bash) et/ou `tools/run_readme_scenario.ps1` (PowerShell) pour exécuter automatiquement le scénario et afficher PASS/FAIL.

Dites-moi si vous voulez que je crée aussi `commands.txt`, `expected.txt`, `Fruit_extra.csv` et `tools/run_readme_scenario.sh` dans le repo — je peux les ajouter et lancer les tests.
